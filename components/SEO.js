...
export const SEO = ({ title, description, type = 'website', images = [] }) => {
  ...
  return (
    <Head>
      ...
      {images.length && images.map(({ url }) => <meta property="og:image" content={url} key={url} />)}

      {images.length && <meta name="twitter:image" content={images[0]?.url} />}
...
export const BlogSEO = ({ authorDetails, title, summary, date, lastmod, url, images }) => {
  ...
  return (
    <>
      <SEO title={title} description={summary} type="article" images={featuredImages} />
      <Head>
